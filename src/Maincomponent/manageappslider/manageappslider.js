...
export default class Dashboard extends Component {
  ...
  handlePaginationChange = (event, { activePage} ) => {
    fetch('http://18.234.207.58:5000/manageapp/'+activePage)
    ...
    console.log("COmponent mounting")
        fetch('http://18.234.207.58:5000/manageapp/')
    .then((response) => response.json())
    .then((responseJson) => {
        console.log(responseJson)
  ...
  addcategoryserver = (cn,cb) => {
     ...
     // data.append('cdescription',cd)
      data.append("img", cb)
      axios.post("http://18.234.207.58:5000/manageapp/addslider/",
      data,
  ...
  editcategoryserver = (cn,cb)  => {
    ...
    fetch("http://18.234.207.58:5000/manageapp/editslider/", {
      method: 'POST',
      body: data,
}).then((response) => {
  this.fetchnewresponse()
  ...
  handleDelete = (id) => {
      let res=[]
      alert(id)
      fetch('http://18.234.207.58:5000/manageapp/deleteslider/', {
        method: 'POST',
        headers: {
