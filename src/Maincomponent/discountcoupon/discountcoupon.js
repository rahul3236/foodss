...
export default class Location extends Component {
  ...
  handlePaginationChange = (event, { activePage} ) => {
    fetch('http://18.191.229.163:5000/discountcoupon/'+activePage)
    ...
    console.log("COmponent mounting")
        fetch('http://18.191.229.163:5000/discountcoupon/')
    .then((response) => response.json())
    .then((responseJson) => {
        console.log(responseJson)
...
.catch((error) => {
  ...
  addwithserver = (t,c,v,dt,dv,ma,cb) => {
    ...
    data.append('img',cb)
    axios.post("http://18.191.229.163:5000/discountcoupon/addcoupon/",
    data,
  ...
  editwithserver = (t,c,v,dt,dv,ma,cb) => {
      ...
      data.append('idtoedit',this.state.idtoedit)
      data.append('img',cb)
      axios.post("http://18.191.229.163:5000/discountcoupon/editcoupon/",
      data,
      {headers:{ 'Content-Type': 'multipart/form-data'}}
  ...
  handleDelete = (id) => {
      let res=[]
      fetch('http://18.191.229.163:5000/sales/deletesales/', {
        method: 'POST',
        headers: {
