...
export default class Location extends Component {
  ...
  handlePaginationChange = (event, { activePage} ) => {
    fetch('http://18.191.229.163:5000/category/'+activePage)
...
fetchnewresponse= () => {
  fetch('http://18.191.229.163:5000/category/')
.then((response) => response.json())
.then((responseJson) => {
  console.log(responseJson)
...
console.error(error);
  ...
  editcategoryserver = (cn,cd,cb)  => {
    ...
    axios.post("http://18.191.229.163:5000/category/edit/",
    data,
    {headers:{ 'Content-Type': 'multipart/form-data'}}
  ...
  handleDelete = (id) => {
      let res=[]
      fetch('http://18.191.229.163:5000/category/delete/', {
        method: 'POST',
        headers: {
