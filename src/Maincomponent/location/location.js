...
export default class Location extends Component {
  ...
  handlePaginationChange = (event, { activePage} ) => {
    fetch('http://18.191.229.163:5000/location/'+activePage,
    {
 mode: 'cors',
 headers: {
   'Access-Control-Allow-Origin':'*'
    ...
    console.log("COmponent mounting")
        fetch('http://18.191.229.163:5000/location')
    .then((response) => response.json())
    .then((responseJson) => {
        console.log(responseJson)
...
console.error(error);
  ...
  editwithserver = (value) => {
    fetch('http://18.191.229.163:5000/location/editlocation/', {
      method: 'POST',
      headers: {
  ...
  handleDelete = (id) => {
      let res=[]
      fetch('http://18.191.229.163:5000/location/deletelocation/', {
        method: 'POST',
        headers: {
          Accept: 'application/json',
          'Content-Type': 'application/json',
...
addwithserver = (value) => {
  alert(value)
  fetch('http://18.191.229.163:5000/location/addlocation/', {
    method: 'POST',
    headers: {
      Accept: 'application/json',
      'Content-Type': 'application/json',
        ...
        handlePaginationChange = {this.handlePaginationChange}
         data={this.state.response} edit={(id)=> this.handleEdit(id)}
            ...
            addlocation={()=>this.addloc()}
             ...
             /> : "Waiting"}
              {this.state.editshow?<EModal hide={()=> this.setState({editshow:false})} editlocation={(value) => this.editwithserver(value)} />:null}
              {this.state.addshow?<AModal hide={()=> this.setState({addshow:false})} addlocationserver={(value) => this.addwithserver(value)} />:null}
    </div>
    )
}
}
